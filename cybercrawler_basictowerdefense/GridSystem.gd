...
func _input(event):
	...
	if event is InputEventMouseButton and event.pressed and event.button_index == MOUSE_BUTTON_LEFT:
		handle_grid_click(event.global_position)
	elif event is InputEventMouseMotion:
		handle_mouse_hover(event.global_position)
...
func world_to_grid(world_pos: Vector2) -> Vector2i:
	var local_pos = world_pos - grid_container.global_position
...
func grid_to_world(grid_pos: Vector2i) -> Vector2:
	...
	return Vector2(world_x, world_y) + grid_container.global_position
...
func _draw():
	...
	if is_valid_grid_position(hover_grid_pos) and not is_grid_occupied(hover_grid_pos):
