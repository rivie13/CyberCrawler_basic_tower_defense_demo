...
func run():
	...
	# IMMEDIATE VALIDATION - Check coverage requirements NOW
...
func _validate_coverage_requirements():
	...
	for script_path in coverage.coverage_collectors:
		...
		if has_tests or has_coverage:
			...
			# Calculate minimum required coverage for this file (50% of file size)
	...
	if validation_errors.size() > 0:
		...
		# Show minimal coverage report (no detailed line-by-line output)
		...
		print("✅ All files with tests meet coverage requirements!")
		
		# Show minimal coverage report (no detailed output)
		Coverage.finalize(Coverage.Verbosity.NONE)
